class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "chattlin-bot-#{conversation.inbox.id}",
      channel_id: "chattlin-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "chattlin-csml-bot-#{agent_bot.id}",
      name: "chattlin-csml-bot-#{agent_bot.id}",
      default_flow: 'chattlin_bot_flow',
      flows: [
        {
          id: "chattlin-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'chattlin_bot_flow',
